# Copyright 2014 John Reese
# Licensed under the MIT license

from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
from __future__ import unicode_literals

import json
import six

safe_types = (bool, int, float, six.text_type, six.binary_type,
              tuple, list, dict, set)


class Encoder(json.JSONEncoder):
    """A simple JSON encoder that looks for objects with a _encode() method,
    and uses that to return their JSON representation."""

    def default(self, o):
        try:
            return o._encode()
        except:
            return str(o)


class Singleton(type):
    """Metaclass for creating singleton objects.
    Requires the class to derive from object.

    Example:

        class Something(object):
            __metaclass__ = Singleton

        # create brand-new Something
        s1 = Something()

        # create a second Something
        s2 = Something()

        # compare the two instances
        assert id(s1) == id(s2)

    """
    _instances = {}

    def __call__(cls, *args, **kwargs):
        obj = Singleton._instances.get(cls, None)

        if obj is None:
            obj = cls.__new__(cls)
            obj.__init__(*args, **kwargs)
            Singleton._instances[cls] = obj

        return obj


class Struct(object):
    """A basic object type that, given a dictionary or keyword arguments,
    converts the key/value pairs into object attributes."""

    def __init__(self, data=None, **kwargs):
        if data is None:
            data = {}
        data.update(kwargs)

        # prevent overwriting values with unsafe callables
        for key, value in list(data.items()):
            if (key in self.__class__.__dict__ and
                    type(value) not in safe_types):
                data.pop(key)

        self.__dict__.update(data)

    def __repr__(self):
        return '<%s %s>' % (self.__class__.__name__, self.__dict__)

    def _encode(self):
        """Generate a recursive JSON representation of the struct."""
        obj = {k: v for k, v in self.__dict__.items()
               if not k.startswith('_') and type(v) in safe_types}
        obj.update({k: v._encode() for k, v in self.__dict__.items()
                   if isinstance(v, Struct)})
        return obj

    def copy(self):
        """Generate a new struct with the same keys and values."""
        return self.__class__.load(self)

    @classmethod
    def load(cls, data):
        """Create a new struct from an existing value.  The value must either
        be an instance of Struct, or must be an instance of safe_types.  If
        the value is a base type (bool, int, string, etc), it will just be
        returned.  Iterable types will be loaded recursively, transforming
        dictionaries into Struct instances, but otherwise maintaining the
        hierarchy of the input data."""
        t = type(data)

        if isinstance(data, Struct):
            return cls({k: cls.load(v) for k, v in data.__dict__.items()})

        elif t not in safe_types:
            return None

        elif t in (tuple, list, set):
            return t(cls.load(i) for i in data)

        elif t == dict:
            return cls({k: cls.load(v) for k, v in data.items()})

        else:
            return data

    @classmethod
    def merge(cls, *args, **kwargs):
        """Create a new Struct from one or more existing Structs.  Keys in the
        later Struct objects will overwrite the keys of the previous Structs.
        Later keys of different type than in earlier Structs will be bravely
        ignored.

        The following keyword arguments are recognized:

        newkeys: boolean value to determine whether keys from later Structs
        should be included if they do not exist in earlier Structs.

        ignore: list of strings of key names that should not be overridden by
        later Struct keys.

        """
        newkeys = bool(kwargs.get('newkeys', False))
        ignore = kwargs.get('ignore', list())

        if len(args) < 1:
            raise ValueError('no structs given to Struct.merge()')
        elif not all(isinstance(s, Struct) for s in args):
            raise ValueError('all positional arguments to Struct.merge() must '
                             'be instances of Struct')

        struct = args[0]
        data = cls.load(struct)

        for struct in args[1:]:
            for key, value in struct.__dict__.items():
                if key in ignore:
                    continue

                if key in data.__dict__:
                    v1 = data.__dict__[key]
                    if type(value) == type(v1):
                        if isinstance(v1, Struct):
                            data.__dict__[key] = cls.merge(v1, value, **kwargs)
                        else:
                            data.__dict__[key] = cls.load(value)

                elif newkeys:
                    data.__dict__[key] = value

        return data

    @classmethod
    def diff(cls, *args, **kwargs):
        """Create a new Struct representing the differences in two or more
        existing Structs.  Keys in the later Structs with values that differ
        from the earlier Structs will be present in the final Struct with the
        latest value seen for that key.  Later keys of different type than in
        earlier Structs will be bravely ignored.

        The following keywoard arguments are recognized:

        newkeys: boolean value to determine whether keys from later Structs
        should be included if they do not exist in earlier Structs.

        ignore: list of strings of key names that will not be included.

        """
        newkeys = bool(kwargs.get('newkeys', False))
        ignore = kwargs.get('ignore', list())

        if len(args) < 2:
            raise ValueError('less than two structs given to Struct.diff()')
        elif not all(isinstance(s, Struct) for s in args):
            raise ValueError('all positional arguments to Struct.diff() must '
                             'be instances of Struct')

        s1 = args[0]
        differences = Struct()

        for s2 in args[1:]:
            for key, value in s2.__dict__.items():
                if key in ignore:
                    continue

                if key in s1.__dict__:
                    v1 = s1.__dict__[key]
                    if type(value) == type(v1):
                        if isinstance(v1, Struct):
                            delta = cls.diff(v1, value, **kwargs)
                            if len(delta.__dict__):
                                differences.__dict__[key] = delta
                        elif v1 != value:
                            differences.__dict__[key] = cls.load(value)

                elif newkeys:
                    differences.__dict__[key] = cls.load(value)

            s1 = s2

        return differences

    @classmethod
    def subclasses(cls):
        """Return a set of all Struct subclasses, recursively."""
        seen = set()
        queue = set([cls])

        while queue:
            c = queue.pop()
            seen.add(c)

            sc = c.__subclasses__()
            for c in sc:
                if c not in seen:
                    queue.add(c)

        seen.remove(cls)
        return seen
